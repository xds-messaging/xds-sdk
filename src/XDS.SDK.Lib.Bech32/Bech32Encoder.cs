...
namespace XDS.SDK.Lib.Bech32
{
    ...
    {
        #region static
        public static readonly byte[] CharacterByteSet = Encoding.ASCII.GetBytes("qpzry9x8gf2tvdw0s3jn54khce6mua7l");

        static readonly int[] Gf1024Exp = {
        ...
        static uint Syndrome(uint residue)
        {
        ...
        static int[] LocateErrors(uint residue, int length)
        {
            if (residue == 0)
            {
                return new int[0];
            ...
            var s1 = (syn >> 10) & 0x3FF;
            var s2 = syn >> 20;
            var l_s0 = Gf1024Log[s0];
            var l_s1 = Gf1024Log[s1];
            var l_s2 = Gf1024Log[s2];
            ...
            {
                var s2_s1p1 = s2 ^ (s1 == 0 ? 0 : Gf1024Exp[(l_s1 + p1) % 1023]);
                if (s2_s1p1 == 0)
                    continue;
                var s1_s0p1 = s1 ^ (s0 == 0 ? 0 : Gf1024Exp[(l_s0 + p1) % 1023]);
                if (s1_s0p1 == 0)
                    continue;
                var l_s1_s0p1 = Gf1024Log[s1_s0p1];
                var p2 = (Gf1024Log[s2_s1p1] - l_s1_s0p1 + 1023) % 1023;
                if (p2 >= length || p1 == p2)
                    continue;
                var s1_s0p2 = s1 ^ (s0 == 0 ? 0 : Gf1024Exp[(l_s0 + p2) % 1023]);
                if (s1_s0p2 == 0)
                    continue;
                var inv_p1_p2 = 1023 - Gf1024Log[Gf1024Exp[p1] ^ Gf1024Exp[p2]];
                var l_e2 = l_s1_s0p1 + inv_p1_p2 + (1023 - 997) * p2;
                if ((l_e2 % 33) != 0)
                    continue;
                var l_e1 = Gf1024Log[s1_s0p2] + inv_p1_p2 + (1023 - 997) * p1;
                if ((l_e1 % 33) != 0)
                    continue;
                if (p1 < p2)
                {
        ...
        static void CheckCase(string hrp)
        {
            if (hrp.Length is 0)
                return;
        ...
        readonly byte[] hrpExpand;

        public Bech32Encoder(string hrp) : this(hrp == null ? null : Encoding.ASCII.GetBytes(hrp.ToLowerInvariant()))
        {
        }

        public Bech32Encoder(byte[] hrp)
        {
            this.HumanReadablePart = hrp ?? throw new ArgumentNullException(nameof(hrp));

            var len = hrp.Length;
            this.hrpExpand = new byte[(2 * len) + 1];
            for (int i = 0; i < len; i++)
            {
                this.hrpExpand[i] = (byte)(hrp[i] >> 5);
                this.hrpExpand[i + len + 1] = (byte)(hrp[i] & 31);
            }
        }

        public byte[] Decode(string address, out byte witnessVersion)
        {
            if (address == null)
                throw new ArgumentNullException(nameof(address));
            CheckCase(address);
            var data = DecodeDataCore(address);
        ...
        public byte[] DecodeToBytes(string address)
        {
            if (address == null)
                throw new ArgumentNullException(nameof(address));
            ...
            var decoded = ConvertBits(data.Skip(1), 5, 8, true);
            if (decoded.Length < 2 || decoded.Length > 40)
                throw new FormatException("Invalid decoded data length");
        ...
        public string Encode(byte witnessVersion, byte[] witnessProgram)
        {
            byte[] data = new[] { witnessVersion }.Concat(ConvertBits(witnessProgram, 8, 5)).ToArray();
        ...
        {
            byte[] data = ConvertBits(bytes, 8, 5);
            var ret = EncodeData(data, 0, data.Length);
            return ret;
        }

        internal string EncodeData(byte[] data, int offset, int count)
        {
            Span<byte> combined = this.HumanReadablePart.Length + 1 + count + 6 is int v && v > 256 ? new byte[v] : stackalloc byte[v];
            int combinedOffset = 0;
            this.HumanReadablePart.CopyTo(combined);
            ...
            {
                var currentIndex = this.HumanReadablePart.Length + 1 + i;
                var currentByte = combined[currentIndex];
                var characterByte = CharacterByteSet[currentByte];
                combined[this.HumanReadablePart.Length + 1 + i] =characterByte;
            }
            return Encoding.ASCII.GetString(combined.ToArray());
        }


        public byte[] HumanReadablePart { get; }

        bool VerifyChecksum(ReadOnlySpan<byte> data, int bechStringLen, out int[] errorPosition)
        {
            errorPosition = null;
            Span<byte> values = this.hrpExpand.Length + data.Length is int v && v > 256 ? new byte[v] : stackalloc byte[v];
            this.hrpExpand.CopyTo(values);
            data.CopyTo(values.Slice(this.hrpExpand.Length));
            var polymod = PolyMod(values) ^ 1;
            if (polymod != 0)
            {
                var epos = LocateErrors(polymod, bechStringLen - 1);
                errorPosition = epos;
                if (epos.Length == 0)
                    return false;
        ...
        void CreateChecksum(byte[] data, int offset, int count, Span<byte> ret)
        {
            Span<byte> values = this.hrpExpand.Length + count + 6 is int v && v > 256 ? new byte[v] :
                stackalloc byte[v];
            var valuesOffset = 0;
            this.hrpExpand.AsSpan().CopyTo(values.Slice(valuesOffset));
            valuesOffset += this.hrpExpand.Length;
        ...
        byte[] DecodeDataCore(string encoded)
        {
            ...
            if (buffer.Length < this.HumanReadablePart.Length)
                throw new FormatException("Mismatching human readable part");
            for (int i = 0; i < this.HumanReadablePart.Length; i++)
            {
                if (buffer[i] != this.HumanReadablePart[i])
                    throw new FormatException("Mismatching human readable part");
            }
            Span<byte> data = encoded.Length - pos - 1 is int v && v > 256 ? new byte[v] : stackalloc byte[v];
            for (int j = 0, i = pos + 1; i < encoded.Length; i++, j++)
            {
                int index = Array.IndexOf(CharacterByteSet, buffer[i]);
                if (index == -1)
