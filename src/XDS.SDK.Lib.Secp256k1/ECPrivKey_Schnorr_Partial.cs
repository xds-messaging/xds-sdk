...
namespace XDS.SDK.Lib.Secp256k1
{
    ...
    /// </summary>
    public class SchnorrNonceFunction : INonceFunction
    {
        readonly byte[]? data;
        ...
        public bool TryGetNonce(Span<byte> nonce32, ReadOnlySpan<byte> msg32, ReadOnlySpan<byte> key32, ReadOnlySpan<byte> algo16, uint counter)
        {
            ...
            if (this.data != null)
            {
                sha.Write(this.data.AsSpan().Slice(0, 32));
            }
            sha.GetHash(nonce32);
            return true;
        }
    ...
    public partial class ECPrivKey
    {
        ...
        public bool TrySignSchnorr(ReadOnlySpan<byte> msg32, INonceFunction? nonceFunction, out bool nonceIsNegated, out SecpSchnorrSignature? signature)
        {
            ...
            ref readonly Scalar x = ref this.sec;
            Scalar e;
            Scalar k;
            GEJ pkj;
            GEJ rj;
