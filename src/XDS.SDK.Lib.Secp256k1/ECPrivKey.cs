...
namespace XDS.SDK.Lib.Secp256k1
{
	...
	*/
	...
	public class PrecomputedNonceFunction : INonceFunction
	{
		...
		{
			this.nonce.AsSpan().Slice(0, 32).CopyTo(nonce32);
			return counter == 0;
		}
	}

	public class RFC6979NonceFunction : INonceFunction
	{
		byte[]? data = null;
	...
	partial class ECPrivKey : IDisposable
	{
		...
		public ECPrivKey(ReadOnlySpan<byte> b32, Context ctx)
		{
			...
			this.sec = new Scalar(b32, out int overflow);
			if (overflow != 0 || this.sec.IsZero)
		...
		public ECPubKey CreatePubKey()
		{
			...
			GEJ pj;
			GE p;
			pj = this.ctx.EcMultGenContext.MultGen(this.sec);
			p = pj.ToGroupElement();
			var pubKey = new ECPubKey(p, this.ctx);
			return pubKey;
		}
		...
		public bool TryTweakAdd(ReadOnlySpan<byte> tweak, out ECPrivKey? tweakedPrivKey)
		{
			...
			if (ret)
			{
				seckey = new ECPrivKey(sec, this.ctx, false);
		...
		public bool TrySignECDSA(ReadOnlySpan<byte> msg32, INonceFunction? nonceFunction, out int recid, out SecpECDSASignature? signature)
		{
			...
			VERIFY_CHECK(!this.sec.IsZero && !this.sec.IsOverflow);
			Span<byte> nonce32 = stackalloc byte[32];
			Span<byte> seckey = stackalloc byte[32];
			this.sec.WriteToSpan(seckey);
			uint count = 0;
			...
			while (true)
			{
				...
				{
					if (secp256k1_ecdsa_sig_sign(this.ctx.EcMultGenContext, out r, out s, this.sec, msg, non, out recid))
					{
						break;
					}
				}
		...
		public bool TryTweakMul(ReadOnlySpan<byte> tweak, out ECPrivKey? tweakedPrivkey)
		{
			...
			if (ret)
			{
				tweakedPrivkey = new ECPrivKey(sec, this.ctx, false);
			}
			Scalar.Clear(ref sec);
