...
namespace XDS.SDK.Lib.Secp256k1
{
	...
	class ECMultContext
	{
		...
		public ECMultContext()
		{
			...
			secp256k1_ecmult_odd_multiples_table_storage_var(ArraySize, this.pre_g, gj);

			{
				...
				secp256k1_ecmult_odd_multiples_table_storage_var(ArraySize, this.pre_g_128, g_128j);
			}
		}
     ...
     */
			...
			for (i = bits - 1; i >= 0; i--)
			{
				...
				{
					ECMULT_TABLE_GET_GE_STORAGE(out tmpa, this.pre_g, n, WINDOW_G);
					r = r.AddZInvVariable(tmpa, Z);
				}
				if (i < bits_ng_128 && (n = wnaf_ng_128[i]) != 0)
				{
					ECMULT_TABLE_GET_GE_STORAGE(out tmpa, this.pre_g_128, n, WINDOW_G);
					r = r.AddZInvVariable(tmpa, Z);
				}
			}
