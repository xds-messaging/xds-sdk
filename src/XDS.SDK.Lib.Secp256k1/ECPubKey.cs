...
{
	public partial class ECPubKey
	{
		public readonly GE Q;
		...
		public void WriteToSpan(bool compressed, Span<byte> output, out int length)
		{
			...
			this.Q.x.WriteToSpan(output.Slice(1));
			if (compressed)
			{
				length = 33;
				output[0] = this.Q.y.IsOdd ? GE.SECP256K1_TAG_PUBKEY_ODD : GE.SECP256K1_TAG_PUBKEY_EVEN;
			}
			else
			{
				length = 65;
				output[0] = GE.SECP256K1_TAG_PUBKEY_UNCOMPRESSED;
				this.Q.y.WriteToSpan(output.Slice(33));
			}
		}
		...
		public bool SigVerify(SecpECDSASignature signature, ReadOnlySpan<byte> msg32)
		{
			...
			var (r, s) = signature;
			return (!s.IsHigh &&
					secp256k1_ecdsa_sig_verify(this.ctx.EcMultContext, r, s, this.Q, m));
		}
 ...
 */
		...
		{
			return new ECPubKey(this.Q.Negate(), this.ctx);
		}
		...
		public override int GetHashCode()
		{
			...
			{
				int hash = 17;
				hash = hash * 23 + this.Q.x.GetHashCode();
				hash = hash * 23 + this.Q.y.GetHashCode();
				return hash;
			}
		}
		...
		public bool TryAddTweak(ReadOnlySpan<byte> tweak, out ECPubKey? tweakedPubKey)
		{
			...
			if (ret)
			{
				if (secp256k1_eckey_pubkey_tweak_add(this.ctx.EcMultContext, ref p, term))
				{
					tweakedPubKey = new ECPubKey(p, this.ctx);
				}
				else
				{
					ret = false;
		...
		public bool TryMultTweak(ReadOnlySpan<byte> tweak, out ECPubKey? tweakedPubKey)
		{
			...
			if (ret)
			{
				if (secp256k1_eckey_pubkey_tweak_mul(this.ctx.EcMultContext, ref p, factor))
				{
					tweakedPubKey = new ECPubKey(p, this.ctx);
				}
				else
		...
		public ECPubKey GetSharedPubkey(ECPrivKey key)
		{
			...
			return new ECPubKey(new GE(pt.x.Normalize(), pt.y.Normalize()), this.ctx);
